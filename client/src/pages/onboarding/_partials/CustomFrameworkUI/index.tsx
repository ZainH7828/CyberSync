...
import { getCategoriesByOrgIdAPI, getSubCategoriesByOrgIdAPI } from "@/pages/api/organization";
...
const CustomFrameworkUI: React.FC<SelectCategoryUIType> = ({
  visibility = false,
  ...
  useEffect(() => {
    ...
    if (context.organizationData.value) {
      getCategoriesByOrgIdAPI(
        context.organizationData.value._id,
        (categories) => {
          context.customCategory.set(categories);
        },
        (err) => {
          console.error(err);
        }
...
export default CustomFrameworkUI;
